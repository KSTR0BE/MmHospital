	...
	<div id="join">
		...
		<div id="joinjoin">
			<form action="/.do" method="post" onsubmit="valid(event)">
					...
					<div>
						<input type="text" class="form-control" id="id" name="ns_id"
					...
					<div>
						...
						<div>
							<input type="password" class="form-control" id="pass"
								name="ns_pw" required placeholder="비밀번호"
						...
						<div>
							<input type="password" class="form-control" id="pass2"
								name="ns_pw" required placeholder="비밀번호 확인"
								pattern="(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[!@#\$%\^&\*\(\)_\+\|]).{8,}"
								style="width: 500px; height: 30px;">
						</div>
					...
					<div>
						<label for="name" class="control-label">이름 <span
							class="rq"> *</span></label>
						<div>
							<input type="text" class="form-control" id="name" name="ns_nm"
					...
					<div class="form-group">
						...
						<div>
							<input type="text"  id="reg" name="ns_reg"
								placeholder="000000" required 
								style="width: 100px; height: 30px;">-
							<input type="password"  name="ns_reg2"  
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<input type="email" class="form-control" id="mail" name="ns_em" 
...
function valid(e) {
	...
	$.ajax({
		type: 'post',
		url: '<%=request.getContextPath()%>/adNurseInsert.do',
		data: data,
...
// 			console.log(response);
			...
			location.href='<%=request.getContextPath()%>/adNurseList.do';
		},
		error: function(xhr){
...
$('#idChk').on('click',function(){
	...
	$.ajax({
		url:'<%=request.getContextPath()%>/adminMypage/adNurseIdChk.jsp',
		data : 'userId=' + userId,
