...
<body>
	...
	<div class="container">
		<form class="form-signin" method="post" action="<%=request.getContextPath() %>/Nlogin.do">
			<br><br><br><br>
			<div id="lg">
			<h3 id="lolo" class="form-signin-heading">간호사 로그인   </h3>
			<select id = "sel" onchange ="f_sel(this)">
				<option value="0">선택</option>
				<option value="admin">의사</option>
				<option value="doctor">관리자</option>
				<option value="nurse">일반회원</option>
			</select>	
			</div>
			<p>
			...
			<button id="nurseBtn" class="btn btn-lg btn-primary btn-block">로그인</button>
		</form>
...
<!-- 		</div> -->
	...
	$('#nurseBtn').on('click', function() {
		...
		$.ajax ({
			type: "POST",
			url: "<%=request.getContextPath()%>/Nlogin.do",
			...
			success : function(response) {
				...
				window.location.href = "<%=request.getContextPath()%>/Nlogin.do;"
			}
		},
