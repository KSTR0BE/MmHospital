...
<%-- 					<form action="<%=request.getContextPath()%>/notice/search.do" method="post"> --%>
					<form action="<%=request.getContextPath()%>/notice.do" method="get">
						<div class="board_search">
							<table>
								...
								<thead>
									...
									<tr>
										<th scope="col">제목</th>
										<td><input type="text" name="ntTit" class="form-control" style="width:90%"/></td>
					...
					<div class="table_default">
						<div class="table_default">
							<table>
								...
								%>
									<tr align="center" height="27">
									  <td><%=str + i %></td>
									  <td><a href="<%=request.getContextPath() %>/notice/detail.do?ntNo=<%=ntList.get(i).getNtNo() %>"><%=ntList.get(i).getNtTit() %></a></td>
							...
							<ul class="btns">
								<li>
									<a href="<%=request.getContextPath() %>/notice/insert.do" class="btn_green">등록</a>
								</li>
							</ul>
							<%
								}
...
<script>
		...
		$('.pagination li').on('click',function(){
			location.href = "<%=request.getContextPath()%>/notice.do?page=" + $(this).text();
		});
		...
		//		alert($('.pagination').children().last() .text());
			...
			location.href = "<%=request.getContextPath()%>/notice.do?page=" + page;
