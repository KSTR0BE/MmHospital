...
<%@page import="kr.or.ddit.suggestions.vo.SuggestionsVO"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	SuggestionsVO sv = (SuggestionsVO)request.getAttribute("sv");
 ...
 <p><img src="/MmHospital/resources/dist/images/sub/sub1/res.jpg"/></p>		 </div>
			...
			<input type='hidden' name='idx' value=''>
							...
							<table>
								...
								<thead>
									...
									<tr>
										<th scope="col">제목</th>
										<td colspan="5">&emsp;<%=sv.getSgTit() %></td>
									</tr>
									
									<tr>
										<th scope="col">작성자</th>
										<td>&emsp;<%=sv.getSgNm() %></td>
										<th scope="col">등록일</th>
										<td>&emsp;<%=sv.getSgDt() %></td>
										<th scope="col">조회수</th>
										<td>&emsp;<%=sv.getSgHit() %></td>
									</tr>
									...
									<tr>
										<th scope="col">내용</th>
										<td colspan="5">&emsp;<%=sv.getSgCon() %></td>
									</tr>
								...
								<br><br><br>
									...
									<table>
										...
										<thead>
									<tr>
										<th scope="col" style="color:#00A796">작성자</th>
										<td>&emsp;관리자</td>
										<th scope="col" style="color:#00A796">등록일</th>
										<td>&emsp;<%=cv.getCmDt() %></td>
									</tr>
								</thead>
							</table>
							</div>
							...
							<div class="table_default">	
									...
									<table>
										...
										<thead>
									<tr>
										<td colspan="6" style="background-color:#f9f9f9; text-align: left; height:40px">&emsp;<%=cv.getCmCon() %></td>
									</tr>
								...
								<ul class="btns">
									<li>
									<%
										if(av != null && cv == null && mv == null && dv == null){
									%>
										<button class="btn_green" onclick="confirm_btn3()">답변작성</button>
									<%
										}if(cv != null && cv.getSgNo().equals(sv.getSgNo()) && mv == null && dv == null){
									%>
										<button class="btn_green" onclick="confirm_btn4()">답변수정</button>
										<button class="btn_green" onclick="confirm_btn5()">답변삭제</button>
									<%
										}
									%>
									<%
										if(mv != null && sv.getMemNo().equals(mv.getMem_no()) || dv != null && sv.getDtNo().equals(dv.getDt_no())){
									%>
	...
	<script>
		...
		function confirm_btn(){
			if(confirm("삭제하시겠습니까?")){
				location.href = "<%=request.getContextPath()%>/suggestions/delete.do?sgNo=" + "<%=sv.getSgNo()%>";
			}
		}
		function confirm_btn1(){
			if(confirm("수정하시겠습니까?")){
				location.href = "<%=request.getContextPath()%>/suggestions/update.do?sgNo=" + "<%=sv.getSgNo()%>";
			}
		}
		function confirm_btn2(){
			location.href = "<%=request.getContextPath()%>/suggestions.do";
		}
		function confirm_btn3(){
				location.href = "<%=request.getContextPath()%>/commentt/insert.do?sgNo=" + "<%=sv.getSgNo()%>";
		}
		function confirm_btn4(){
			if(confirm("수정하시겠습니까?")){
				location.href = "<%=request.getContextPath()%>/commentt/update.do?sgNo=" + "<%=sv.getSgNo()%>";
			}
		}
		function confirm_btn5(){
			if(confirm("삭제하시겠습니까?")){
				location.href = "<%=request.getContextPath()%>/commentt/delete.do?sgNo=" + "<%=sv.getSgNo()%>";
			}
		}
